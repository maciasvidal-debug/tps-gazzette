        ...
        >
          <div className="space-y-4">
            <FormInput
              ...
              onChange={e => updateState(draft => {
                if (!draft.feedback) draft.feedback = { question: '', options: [] };
                draft.feedback.question = e.target.value;
              })}
            />
            ...
            <div className="space-y-4">
              ...
              {(state.feedback?.options || []).map((opt, index) => (
                <div key={opt.id} className="p-3 bg-[#1A1A1E] rounded-md border border-[#343541] relative">
                  <button
                    className="absolute top-2 right-2 text-[#8B8D98] hover:text-[#ED6A5E]"
                    onClick={() => updateState(draft => {
                      if (draft.feedback?.options) {
                        draft.feedback.options.splice(index, 1);
                      }
                    })}
                  >
                    ×
                  ...
                  <div className="space-y-3 mt-4">
                    <FormInput
                      ...
                      value={opt.label}
                      onChange={e => updateState(draft => {
                        if (draft.feedback?.options?.[index]) {
                          draft.feedback.options[index].label = e.target.value;
                        }
                      })}
                    />
                    <FormInput
                      ...
                      onChange={e => updateState(draft => {
                        if (draft.feedback?.options?.[index]) {
                          draft.feedback.options[index].link = e.target.value;
                        }
                      })}
                    />
                  </div>
              ...
              <button
                className="w-full py-2 bg-[#343541] hover:bg-[#4B4C56] text-[#D0D1D8] text-sm font-medium rounded transition-colors"
                onClick={() => updateState(draft => {
                  if (!draft.feedback) draft.feedback = { question: 'Was this edition helpful?', options: [] };
                  draft.feedback.options.push({
                    id: Date.now().toString(),
                    label: 'New Option',
                    link: 'https://'
                  });
